...
31개 슬라이드 템플릿이 들어 있고, Claude가 내용에 맞게 알아서 골라줍니다. 파워포인트 감각이 없어도, 디자인을 몰라도 됩니다.
...
Playwright Chromium 등이 자동으로 깔립니다. 2~3분 걸려요.

### 4단계. 시스템 도구 설치 (선택)

슬라이드를 이미지로 미리보고 싶을 때만 필요합니다. `.pptx` 생성 자체에는 없어도 됩니다.
...
### 6단계. Claude Code 재시작 ← 꼭 해야 해요

설치 후 Claude Code를 완전히 껐다 켜야 플러그인이 로드됩니다. 채팅창에 `/k-proposal` 입력했을 때 자동완성이 뜨면 성공입니다.
...
**레이아웃이 마음에 안 들면요?**
"이 슬라이드 레이아웃 바꿔줘"라고 하면 됩니다. 31개 중 다른 걸 골라 다시 그려줘요.
...
├── skills/proposal/        ← PPT 엔진 (31개 템플릿)
│   ├── layouts/
│   ├── components/
