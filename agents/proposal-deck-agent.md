...
| 자산 | 경로 |
|---|---|
| 디자인 절대 규칙 (SKILL.md) | `${PLUGIN_ROOT}/skills/proposal/SKILL.md` |
...
| 폰트 | `${PLUGIN_ROOT}/skills/proposal/fonts/PretendardVariable.ttf` |
| 결정 트리 카탈로그 (스테이지 분할) | `${PLUGIN_ROOT}/skills/proposal/catalog/_index.md` 와 `catalog/stage-*.md` |
| 결정 트리 카탈로그 (전체, 백업) | `${PLUGIN_ROOT}/skills/proposal/CATALOG.md` |
...
`${PLUGIN_ROOT}/skills/proposal/catalog/_index.md` 를 Read 한다. 이 인덱스는 짧은 파일(~70줄) 로, 한국식 아크의 슬롯별로 어느 stage 파일을 봐야 하는지만 알려준다.

```
slot ① 표지 / ⑩ 마무리 → catalog/stage-cover.md
slot ② 목차            → catalog/stage-toc.md
slot ③ 분석 / ⑤ 인사이트 → catalog/stage-analysis.md
slot ⑥ 실행            → catalog/stage-execution.md
slot ⑦ 인력            → catalog/stage-people.md
slot ⑧ 예산            → catalog/stage-budget.md
slot ⑨ 정당화          → 위 stage 파일들 재사용
```

**stage 파일은 Step 3 의 스토리 아크가 확정된 뒤 Step 4 에서 슬라이드 슬롯에 진입할 때 그 1개만** Read 한다. 전체 1300줄짜리 `CATALOG.md` 를 한 번에 로드하지 않는다 — 컨텍스트 토큰 절약이 목적.

각 stage 파일 항목 구조:

- **File**: 어느 HTML 파일을 쓸지
- **Use when**: 후보로 추리는 조건
- **Don't use when**: 비슷한 다른 템플릿과의 분리 규칙(MECE)
- **Tokens**: 채워야 할 토큰명 (간단 나열)
- **Layout**: 시각 규칙 / 절대 규칙과의 정합성

**카탈로그에 없는 템플릿명·토큰명을 발명하지 않는다.** 새 템플릿이 필요하면 사용자에게 알리고, 빌드는 카탈로그에 있는 자산만으로 진행한다.

> stage 파일에 토큰 상세가 부족하면 그 템플릿의 HTML 파일 USAGE 주석을 직접 Read 한다 (단일 슬라이드만). 전체 `CATALOG.md` 로 돌아갈 필요 없음.
...
③ 분석 슬라이드 1~5장
     ...
     · matrix-2x2 / bubble-chart / concept-tree /
       content-grid / content-2-col-cards / content-image-table
④ 핵심 메시지       (한 줄짜리 결론)
     · emphasis-hero 또는 divider 변형 / qslide(pages 사례)
⑤ 핵심 인사이트     emphasis-hero
     · 메시지를 떠받치는 3~5개 인사이트
⑥ 세부 실행방안 슬라이드 N장 (보통 3~6장)
     · timeline / gantt-chart / step-cards /
       numbered-steps-split / numbered-circle-list
⑦ 인력 구성         org-chart / numbered-circle-list
⑧ 예산안            content-image-table 류 / content-grid
⑨ 정당화 슬라이드 1~3장
     · 과거 성공사례   content-grid / content-3-col-cards
     · 자사 솔루션     tool-card-grid
     · 팀 프로필       instructor-profile
⑩ 마무리 / Q&A      divider 또는 emphasis-hero
```

목표 매수 12~18장 (한국 제안서 톤은 mckinsey 식 5~10장보다 상세하다). 매 슬라이드는 **하나의 메시지**만 전달한다. 두 개 이상 메시지가 들어가려 하면 두 슬라이드로 쪼갠다.

### Step 4. 슬라이드별 템플릿 + 1줄 근거 (stage 파일 단위 Read)

각 슬라이드 슬롯이 정해지면 _index.md 가 가리키는 stage 파일 1개를 Read 한다 (이미 같은 stage 를 본 적 있으면 다시 안 읽음). stage 파일의 **Use when** 으로 1~3개 후보를 추리고, **Don't use when** 으로 탈락시킨 뒤 한 개를 선택한다. 결정마다 **한 줄 근거**를 적는다 — 형식은 항상 다음과 같다.
...
`--no-lint` 옵션으로 lint 건너뛰기 가능. 폰트 PretendardVariable.ttf 는 stage 파일에서 사용한 템플릿이 자동 임베드.
